...
public static Integer letCustomerDesConverToInt(Customer customer) {
	...
	public static void finishingMatch(Customer customer) throws InterruptedException {
		if ((contrastAllowDistance(customer) == true)
				&&(drivers.get(0).permitRequestion()==true)) {
			System.out.println();
			System.out.println("[Customer]");
			System.out.println("잠시만 기다려주세요 택시가 출발했습니다");
			System.out.println();
			Thread.sleep(1000);
			System.out.println("기사의 현재 위치는 "+drivers.get(tmp).getLocation()+" 이며, 수학적 위치는 "+drivers.get(tmp).getLoc()+" 입니다.");
			alarmMovingTaxi(customer);
		}
		else if((contrastAllowDistance(customer) == true)
				&&(drivers.get(0).permitRequestion()==false)){
			Thread.sleep(1000);
	...
	public static void alarmMovingTaxi(Customer customer) throws InterruptedException {
		...
		for(int i = taxiToCus; i >= 0; i--) {
			System.out.println("택시가 고객님에게 가고 있습니다. 남은 거리/시간: "+ i);
			Thread.sleep(800);
		}	
		...
		for(int i = CusToDes; i >= 0; i--) {
			System.out.println("택시가 목적지를향해 가고 있습니다. 남은 거리/시간: "+i);
			Thread.sleep(800);
		}	
	...
	public static void payMoeny(Customer customer) {
		int CusToDes = Math.abs(letCustomerLocConverToInt(customer) - letCustomerDesConverToInt(customer));
