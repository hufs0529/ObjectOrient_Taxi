...
public class TaxiManager {
	...
	public TaxiManager() {
		...
		List<String> locs = Arrays.asList("선릉","독산","홍대","죽전","판교","정자","잠실","삼성","강남","역삼","교대","당고개","왕십리","아현","신촌");
		List<String> names = Arrays.asList("김정훈","김윤식","함경녕","조재은","정사도","조길환","김병찬","임은혜","염래헌","김준태","정준","인찬휘","김가람","김효정","김다함","왕재준");
		...
		Customer customer = new Customer(locs.get(randomLocIndex),locs.get(randomDesIndex), names.get(randomNameIndex),(float)Math.round(Math.random()*5) ,(int)(Math.random()*20+1)*1000);
		...
		Driver driver = new Driver(locs.get(randomTaxiIndex), 0, names.get(randomTNameIndex), (float)Math.round(Math.random()*5), (int)Math.random()*15+1, (int)Math.random()*15+1);
...
//		
//		Customer[] customerList = {jeongHun};
//		customers = new ArrayList<>(Arrays.asList(customerList));
		
		Driver byeongChan = new Driver("정자",0, "김병찬", (float) 5.0, 0, 0);
